...
# Credit: Reused completely from https://github.com/barronalex/Dynamic-Memory-Networks-in-TensorFlow/blob/master/babi_input.py
# which was adapted from https://github.com/YerevaNN/Dynamic-memory-networks-in-Theano/
def init_babi(fname):
...
def load_babi(config, split_sentences=False):
    ...
    train_data = process_input(babi_train_raw, config.dtype, word2vec, vocab, ivocab, config.embed_size, split_sentences)
    print('==> get test inputs')
    test_data = process_input(babi_test_raw, config.dtype, word2vec, vocab, ivocab, config.embed_size, split_sentences)
